...
&emsp; &emsp;我们保持BookInterface，Book以及BookShop三个类不变，为了消除Main函数的硬编码问题，引入spring框架来帮我们完成对象的创建工作，而不在代码中使用new Book()的方式显式创建
...
&emsp; &emsp;设计一个打折接口BookInterface，新创一个BookWithOffRate类继承自Book类并实现BookInterface接口的函数，拦截Book类的getPrice方法重新计算价格
...
&emsp; &emsp;为了达到促销打折的功能，基于原来的代码增加编译了三个类，修改bookshop.xml配置文件重新运行，达到了预期的效果，并遵循了开闭原则且在期望的地方消除了硬编码。
